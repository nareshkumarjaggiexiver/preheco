...
def test_the_scope_bridge_suspends_only_the_table_never_the_gate(kit):
    ...
    path, but a credential is still demanded — the bridge must never widen
    into fail-open."""
    env = {
        "HECO_REQUIRE_AUTH": "1",
        "HECO_SCOPE_ENFORCE": "0",
        "HECO_JWKS_JSON": json.dumps(kit.jwks),
        "HECO_AUTH_ISSUER": ISS,
    }
    g = gate_with(env)
    report = kit.mint(iss=ISS, extra_claims={"scope": "planner:report"})
    assert call(g, path="/models/apply", method="POST",
                headers={"authorization": f"Bearer {report}"})[0] == 200
    assert call(g, path="/models/apply", method="POST")[0] == 401, (
        "no credential is still no entry — the bridge only suspends scopes"
    )
