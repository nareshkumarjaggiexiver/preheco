...
class MatchClient:
    ...
    def match(
        ...
        *,
        ...
        one.  ``skin`` (2) is the light the face was read under (match
        0.15.0 holds a colour set-aside back when two identities' skin
        disagrees).  Each is sent only when known, like ``appearance``:
        absent is not zero, and a null on the wire says nothing an omitted
        key does not.
        """
        wire: dict = {
...
class LoopObserver:
    """Adapts a RunLoop to :class:`heco_counting.ports.CountingObserver`.

    Three one-line forwards. It exists so the library can be given somewhere
    to report WITHOUT being handed the run loop itself — a worker and a fusion
    process will each write their own, and neither wants a RunLoop.
    """
...
class RunLoop:
    ...
    def __init__(
        ...
        self.reporter = None
        # This loop seen through heco_counting's observer protocol: the
        # library reports what it did and never reads anything back, so a
        # decision can never depend on whether observability is switched on.
        self._observer = LoopObserver(self)
    ...
    def _pipeline_step(
        ...
        for face, emb, norm, attr in zip(kept, embeddings, norms, attrs, strict=False):
            ...
            # HEAD, BEARD AND SKIN, for the review queue alone: read off the
            # same frame from the face's landmarks (a face without them has
            # none; absent is not zero) under the same white balance.  The
            # skin is the LIGHT the colours were read under: match holds a
            # colour set-aside back when two identities' skin disagrees.
            head = beard = skin = None
            ...
            if frame_img is not None and landmarks:
                ...
                skin = appearance.skin_tone(frame_img, landmarks, wb_gains)
    ...
    def _resolve_excluding(
        ...
        # beard, skin}), each only when known — the omit-when-absent rule again.
    ...
    def _apply_zones(self, faces: list, frame: dict) -> list:
        """Delegates to :func:`heco_counting.zones.apply_face_zones`."""
        return zones.apply_face_zones(faces, frame, self.zones, self._observer)
    ...
    def _mark_person_zones(self, boxes: list, frame: dict) -> None:
        ...
        zones.mark_person_zones(boxes, frame, self.zones, self._observer)
